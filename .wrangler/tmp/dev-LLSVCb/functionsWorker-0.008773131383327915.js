...
// .wrangler/tmp/pages-WPFEP5/functionsWorker-0.008773131383327915.mjs
...
var init_compose = __esm({
  "../node_modules/hono/dist/compose.js"() {
    init_functionsRoutes_0_6335629700847167();
    ...
    bulkRouter.get("/admin/stats", async (c) => {
      ...
      try {
        const [
          ...
          // 1. Pending Orders
          db.select({ value: count() }).from(orders).where(or(
            eq(orders.status, "Pending"),
            eq(orders.status, "Pending Verification"),
            eq(orders.status, "Processing")
          )),
          // 2. Pending Returns
          db.select({ value: count() }).from(returns).where(eq(returns.status, "Requested")),
          // 3. Pending Reviews
          db.select({ value: count() }).from(reviews).where(eq(reviews.status, "pending")),
          // 4. Unread Admin Notifications
          db.select({ value: count() }).from(notifications).where(and(
...
var routes;
var init_functionsRoutes_0_6335629700847167 = __esm({
  "../.wrangler/tmp/pages-WPFEP5/functionsRoutes-0.6335629700847167.mjs"() {
    init_app();
    init_route();
    init_slug();
    routes = [
...
// .wrangler/tmp/bundle-IdNxYw/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-IdNxYw/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.008773131383327915.js.map
