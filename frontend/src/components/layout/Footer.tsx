...
import { Mail, ArrowRight, MapPin, Phone, Heart, CheckCircle, Loader2 } from 'lucide-react';
...
export const Footer: React.FC = () => {
  ...
  const handleNewsletterSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = (err as any)?.response?.data?.message || 'Failed to subscribe. Please try again.';
      toast.error(msg);
    } finally {
    ...
    <footer className="bg-[#FFFBF0] text-gray-900 pt-10 pb-4 mt-auto rounded-t-[2.5rem] border-t-2 border-[#FF4500]">
      <div className="container mx-auto px-4 md:px-8">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <span className="text-[10px] font-bold uppercase tracking-[0.3em] text-[#B8860B]/60">Aurelia</span>
          <div className="h-px flex-1 bg-gradient-to-r from-transparent via-[#B8860B] to-transparent opacity-40" />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-6">
          ...
          <div className="md:col-span-5 bg-black/[0.02] border border-[#FF4500]/15 rounded-2xl p-5 flex flex-col justify-between group hover:border-[#FF4500]/40 transition-all duration-300 shadow-sm">
            <div>
              <div className="flex items-center gap-3 mb-3">
                ...
                />
                <h2 className="text-xl font-bold text-gray-900 font-garamond tracking-tight">Aurelia</h2>
              </div>
          ...
          <div className="md:col-span-7 grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            {/* Quick Links + Categories — 2 columns on mobile, 1 col each on desktop */}
            ...
            <div className="bg-gradient-to-br from-[#FF4500]/5 to-transparent border border-[#FF4500]/15 rounded-2xl p-5 hover:border-[#FF4500]/40 transition-all duration-300 shadow-sm">
              ...
              {subscribed ? (
                <div className="flex items-center gap-2 py-3 px-4 rounded-xl bg-green-50 border border-green-200 text-green-700 text-xs font-bold animate-in zoom-in-95 duration-300">
                  <CheckCircle size={14} />
                ...
                <form onSubmit={handleNewsletterSubmit} className="flex flex-col gap-2">
                  ...
                  <button 
                    ...
                    className="w-full bg-[#FF4500] hover:bg-[#E63E00] text-white px-3 py-2 rounded-xl font-bold text-[10px] uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-lg shadow-[#FF4500]/20 hover:shadow-[#FF4500]/30 hover:-translate-y-0.5 active:translate-y-0 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {subscribing ? (
                      <>
        ...
        <div className="bg-black/[0.02] border border-[#FF4500]/10 rounded-2xl px-5 py-3 flex flex-col md:flex-row justify-between items-center gap-2">
          <p className="text-gray-500 text-[10px] flex items-center gap-1.5">
            &copy; {new Date().getFullYear()} Aurelia. Made with <Heart size={10} className="text-[#FF4500] fill-[#FF4500]" /> in BD
          </p>
