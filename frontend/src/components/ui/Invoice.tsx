...
export const Invoice: React.FC<InvoiceProps> = ({ order, onClose, autoPrint }) => {
  const isMobile = window.innerWidth < 768;

  React.useEffect(() => {
    if (autoPrint && isMobile) {
      // Small timeout to ensure styles are applied before print dialog opens
      const timer = setTimeout(() => {
        window.print();
        onClose();
      }, 500); // Slightly longer timeout to allow user to see the loading state
      return () => clearTimeout(timer);
    }
  }, [autoPrint, isMobile, onClose]);

  if (!order) return null;

  return createPortal(
    <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 invoice-modal-overlay 
      ${autoPrint && isMobile ? 'pointer-events-none' : 'bg-black/60 backdrop-blur-sm'}`}
      style={autoPrint && isMobile ? { visibility: 'hidden' } : {}}
    >
      <div className="bg-white rounded-3xl w-full max-w-3xl max-h-[90vh] flex flex-col shadow-2xl relative invoice-card-print"
        style={autoPrint && isMobile ? { visibility: 'visible', position: 'absolute', left: '-9999px' } : {}}
      >
        {/* Header Actions */}
        ...
        <div className="p-8 overflow-y-auto flex-grow printable-invoice">
          <div className="flex justify-between items-start mb-12">
            <div>
              <h1 className="text-3xl font-black font-garamond text-primary mb-2">Aurelia</h1>
              <p className="text-gray-500 text-sm">Dhaka, Bangladesh<br/>support@aurelia.com<br/>+880 1XXX-XXXXXX</p>
            </div>
        ...
        <style>{`
          @media print {
            ...
            /* Hide the main app root completely during print */
            #root {
              display: none !important;
            }
