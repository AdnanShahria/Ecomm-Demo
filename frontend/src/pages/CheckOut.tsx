...
export const CheckOut: React.FC = () => {
  ...
  const hasFetchedAddressesRef = React.useRef(false);
  useEffect(() => {
    if (isAuthenticated && user?.id && !hasFetchedAddressesRef.current) {
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-grow space-y-3">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
            <h3 className="text-lg font-bold text-primary mb-3">Shipping Details</h3>
            ...
            <form className="space-y-4">
              <div className="grid grid-cols-2 gap-4 mb-4">
                <button
                  ...
                  className={`py-1.5 px-3 rounded-xl border text-xs font-bold transition-all ${shippingZone === 'dhaka' ? 'border-primary bg-primary/5 text-primary' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}
                >
                  Inside Dhaka (৳60)
                </button>
                <button
                  ...
                  className={`py-1.5 px-3 rounded-xl border text-xs font-bold transition-all ${shippingZone === 'outside' ? 'border-primary bg-primary/5 text-primary' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}
                >
                  Outside Dhaka (৳120)
                </button>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input 
                    ...
                    className="w-full px-3 py-2 rounded-xl bg-gray-50 border border-gray-200 text-sm focus:border-accent focus:ring-1 focus:ring-accent outline-none transition-all"
              ...
              <div>
                <label className="block text-xs font-bold text-primary mb-1">Shipping Address *</label>
                <textarea 
                  name="shippingAddress"
                  value={formData.shippingAddress}
                  onChange={handleInputChange}
                  rows={2}
                  className="w-full px-3 py-2 rounded-xl bg-gray-50 border border-gray-200 text-sm focus:border-accent focus:ring-1 focus:ring-accent outline-none transition-all resize-none"
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
            ...
            <div className="grid grid-cols-3 gap-2 mb-3">
              <button
                ...
                className={`py-2.5 px-2 rounded-xl border text-xs sm:text-sm font-bold transition-all ${paymentMethod === 'cod' ? 'border-primary bg-primary text-white shadow-md' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}
              >
                Cash on Delivery
              </button>
              <button
                ...
                className={`py-2.5 px-2 rounded-xl border text-xs sm:text-sm font-bold transition-all ${paymentMethod === 'bkash' ? 'border-[#e2136e] bg-[#e2136e] text-white shadow-md' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}
              >
                bKash
              </button>
              <button
                ...
                className={`py-2.5 px-2 rounded-xl border text-xs sm:text-sm font-bold transition-all ${paymentMethod === 'nagad' ? 'border-[#f7941d] bg-[#f7941d] text-white shadow-md' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}
              >
                Nagad
              </button>
            </div>
            ...
            {paymentMethod !== 'cod' && (
              <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4 space-y-3 mb-3">
                <div className="text-xs text-orange-800">
                  <p className="font-bold mb-0.5 uppercase tracking-wider text-[10px] opacity-70">Payment Instructions:</p>
                  <p className="font-medium text-sm mb-1">Please Send Money to our Personal Number:</p>
                  ...
                  <p className="mt-1 font-bold">Total Amount: <span className="text-lg tracking-tight">৳{finalPayable.toLocaleString()}</span></p>
                </div>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2 border-t border-orange-100">
                  <div>
                    <label className="block text-[9px] font-bold text-orange-900 mb-1 uppercase tracking-wider">Your {paymentMethod.toUpperCase()} Number *</label>
                    <input 
                      type="text" 
                      value={paymentPhone}
                      onChange={(e) => setPaymentPhone(e.target.value)}
                      className="w-full px-3 py-2 rounded-xl bg-white border border-orange-200 text-sm focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none transition-all"
                      placeholder="01XXXXXXXXX"
                    />
                  </div>
                  <div>
                    <label className="block text-[9px] font-bold text-orange-900 mb-1 uppercase tracking-wider">Transaction ID (TrxID) *</label>
                    <input 
                      type="text" 
                      value={paymentTrxId}
                      onChange={(e) => setPaymentTrxId(e.target.value)}
                      className="w-full px-3 py-2 rounded-xl bg-white border border-orange-200 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 outline-none transition-all uppercase tracking-widest font-mono text-sm"
        ...
        <div className="w-full lg:w-[360px] flex-shrink-0">
          <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-4 sticky top-24">
            <h2 className="text-lg font-black font-garamond text-primary mb-3">Order Summary</h2>
            
            {/* Order Details (Selected Items) */}
            <div className="mb-3 space-y-2 max-h-[220px] overflow-y-auto pr-1 custom-scrollbar">
              <h4 className="text-[9px] font-black uppercase text-muted tracking-widest mb-1">Items being ordered</h4>
              {selectedItems.map((item) => (
                <div key={item.product.id} className="flex gap-2 items-center pb-1.5 border-b border-gray-50 last:border-0">
            ...
            <div className="space-y-2 text-xs mb-4 pt-3 border-t border-gray-100">
              ...
              {couponApplied ? (
                <div className="flex items-center justify-between px-3 py-2 bg-green-50 border border-green-200 rounded-xl">
                  <span className="flex items-center gap-2 text-xs font-bold text-green-700">
                    <CheckCircle size={14} /> {couponApplied} applied
                  </span>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-grow px-3 py-2 rounded-xl bg-gray-50 border border-gray-200 text-xs font-bold focus:border-accent focus:ring-1 focus:ring-accent outline-none uppercase tracking-wider"
                    placeholder="PROMO CODE"
                  />
                  <button
                    onClick={handleApplyCoupon}
                    disabled={couponLoading || !couponCode.trim()}
                    className="px-3 py-2 bg-primary text-white rounded-xl font-bold text-xs hover:bg-primary/90 transition-colors disabled:opacity-50 flex items-center gap-1.5"
                  >
            ...
            <button 
              ...
              className="w-full py-3 bg-accent text-white rounded-xl font-bold text-sm hover:bg-accent/90 transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed shadow-lg shadow-accent/20"
            >
              {isSubmitting ? (
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 animate-in fade-in duration-300">
      ...
      <div className="relative bg-white w-full max-w-lg rounded-t-[2rem] sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh] animate-in slide-in-from-bottom-full duration-500">
        <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <h3 className="text-lg font-black font-garamond text-primary">Order Details</h3>
        ...
        <div className="p-5 bg-gray-50/80 border-t border-gray-100 space-y-2.5">
          ...
          <button 
            onClick={onClose}
            className="w-full py-3.5 mt-1 bg-primary text-white rounded-xl font-bold shadow-lg shadow-primary/10 transition-transform active:scale-95"
          >
            Got it
          </button>
        </div>
