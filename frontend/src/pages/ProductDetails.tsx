...
export const ProductDetails: React.FC = () => {
    ...
    const fetchBulkData = async () => {
      ...
      try {
        ...
        setError({
          ...
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          message: (err as any).response?.data?.error || (err as any).message || 'Failed to load product'
        });
      } finally {
        setLoading(false);
      }
  ...
  const tabIds = ['description', 'specs', 'reviews'] as const;
  useEffect(() => {
  ...
  if (error && error.status !== 404) return (
    <div className="min-h-screen flex items-center justify-center bg-white">
      <div className="text-center space-y-6 max-w-md px-6">
        ...
        <h1 className="text-4xl font-black text-gray-900 tracking-tighter">Something went wrong</h1>
        <p className="text-gray-500 font-medium">{error.message || 'We encountered an error while loading this product. Please try again later.'}</p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <button 
            onClick={() => window.location.reload()} 
            className="inline-flex items-center justify-center gap-2 bg-gray-900 text-white px-8 py-4 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-black transition-all"
          >
            Retry Now
          </button>
          <Link to="/shop" className="inline-flex items-center justify-center gap-2 bg-gray-100 text-gray-900 px-8 py-4 rounded-2xl font-black text-sm uppercase tracking-widest hover:bg-gray-200 transition-all">
            Browse Shop
          </Link>
        </div>
      </div>
  ...
  if (!product || (error && error.status === 404)) return (
    <div className="min-h-screen flex items-center justify-center bg-white">
      ...
      <div className="container mx-auto px-4 lg:px-6 pt-6">
        ...
        <div className="mt-12 md:mt-20 space-y-6 md:space-y-8">
          <div className="flex justify-center">
            <div ref={tabBarRef} className="relative inline-flex items-center gap-1 bg-gray-50/80 backdrop-blur-sm border border-gray-100 rounded-2xl p-1 md:p-1.5 shadow-sm">
              {/* Sliding indicator */}
              <div
                className="absolute top-1 md:top-1.5 bottom-1 md:bottom-1.5 rounded-xl bg-white shadow-md shadow-gray-200/50 transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] z-0"
                style={{ left: `${indicatorStyle.left}px`, width: `${indicatorStyle.width}px` }}
              />
              {[
                { id: 'description' as const, label: 'Overview', mobileLabel: 'Overview', icon: Info },
                { id: 'specs' as const, label: 'Specifications', mobileLabel: 'Specs', icon: ListChecks },
                { id: 'reviews' as const, label: `Reviews (${reviewStats?.totalReviews ?? 0})`, mobileLabel: `Reviews (${reviewStats?.totalReviews ?? 0})`, icon: MessageSquare }
              ].map((tab, i) => (
                <button
                  key={tab.id}
                  ref={(el) => { tabRefs.current[i] = el; }}
                  onClick={() => setActiveTab(tab.id)}
                  className={`relative z-10 flex items-center gap-1.5 md:gap-2 px-4 md:px-6 py-2 md:py-2.5 rounded-xl text-[10px] md:text-xs font-black uppercase tracking-wider transition-all duration-300 whitespace-nowrap ${
                    activeTab === tab.id
                      ? 'text-primary'
                      : 'text-gray-400 hover:text-gray-600'
                  }`}
                >
                  <tab.icon size={13} className={`transition-colors duration-300 ${activeTab === tab.id ? 'text-primary' : 'text-gray-300'}`} />
          ...
          <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700">
            {activeTab === 'description' && (
              <div className="bg-white rounded-2xl md:rounded-[2.5rem] border border-gray-100 md:border-2 md:border-gray-200/60 p-5 md:p-12 shadow-sm md:shadow-xl md:shadow-gray-100/50 animate-in fade-in zoom-in-95 duration-500">
                <div className="space-y-5 md:space-y-8 text-center md:text-left">
                  <h2 className="text-xl md:text-3xl font-black text-gray-900 tracking-tight">The Perfect Blend of Safety and Fun</h2>
                  <div className="text-sm md:text-lg text-gray-500 leading-relaxed font-medium whitespace-pre-wrap">
                  ...
                  {product.highlights && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 pt-5 md:pt-8">
                      {(() => {
                        try {
                          const highlights = JSON.parse(product.highlights);
                          // eslint-disable-next-line @typescript-eslint/no-explicit-any
                          return highlights.map((h: any, i: number) => (
                            <div key={i} className="bg-gray-50/50 border border-gray-100 rounded-2xl md:rounded-3xl p-4 md:p-6 text-center hover:bg-white hover:shadow-xl hover:-translate-y-1 transition-all duration-500 group">
                              <div className="w-10 h-10 md:w-12 md:h-12 bg-white rounded-xl md:rounded-2xl flex items-center justify-center text-primary mx-auto mb-3 md:mb-4 shadow-sm group-hover:bg-primary group-hover:text-white transition-colors">
            ...
            {activeTab === 'specs' && (
              <div className="space-y-8">
                {product.specification ? (
                  <div className="bg-white rounded-2xl md:rounded-[2.5rem] border border-gray-100 md:border-2 md:border-gray-200/60 p-5 md:p-12 shadow-sm md:shadow-xl md:shadow-gray-100/50">
                    <div className="text-sm text-gray-600 leading-relaxed font-bold whitespace-pre-wrap">
                      {product.specification}
                    </div>
                  </div>
                ) : (
                  <div className="bg-white rounded-2xl md:rounded-[2.5rem] border border-gray-100 md:border-2 md:border-gray-200/60 overflow-hidden shadow-sm md:shadow-xl md:shadow-gray-100/50">
              ...
              <div className="space-y-6 md:space-y-10">
                {loadingReviews ? (
                  <div className="space-y-4 md:space-y-6">
                    <div className="bg-white p-5 md:p-8 rounded-2xl md:rounded-3xl border border-gray-100 flex gap-10 h-32 md:h-40 skeleton" />
                    {[1, 2, 3].map(i => (
                      <div key={i} className="bg-white p-5 md:p-8 rounded-2xl md:rounded-3xl border border-gray-100 h-36 md:h-48 skeleton" />
                    ))}
                  </div>
                ) : reviews.length > 0 ? (
                  <div className="grid grid-cols-1 gap-5 md:gap-8">
                    ...
                    <div className="space-y-5 md:space-y-6">
                      {reviews.map((review) => {
                        ...
                        return (
                          <div key={review.id} className="bg-gradient-to-br from-white to-gray-50/30 p-5 md:p-8 rounded-2xl md:rounded-[2rem] border border-gray-100 md:border-2 md:border-gray-200/60 shadow-sm hover:shadow-xl transition-all duration-500 group/card">
                            {/* Header: Avatar + Name + Stars + Date */}
                            <div className="flex items-start justify-between gap-3 mb-4">
                              <div className="flex items-center gap-3">
                                <div className="w-11 h-11 md:w-10 md:h-10 rounded-full md:rounded-xl bg-gradient-to-br from-primary to-primary/70 flex items-center justify-center text-white font-black text-sm uppercase ring-2 ring-primary/20 ring-offset-2 shadow-md shadow-primary/10">
                                  {review.username[0]}
                                </div>
                                <div className="space-y-1">
                                  <div className="flex items-center gap-2 flex-wrap">
                                    ...
                                    {review.isVerified === 1 && (
                                      <span className="inline-flex items-center gap-1 text-[8px] font-black text-emerald-600 bg-emerald-50 border border-emerald-100 px-2 py-0.5 rounded-full uppercase tracking-wider">
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-[10px] font-black text-gray-900 ml-0.5">{Number(review.rating).toFixed(1)}</span>
                                  </div>
                                </div>
                              </div>
                              <span className="text-[9px] md:text-[10px] font-bold text-gray-300 uppercase tracking-tight whitespace-nowrap pt-1">
                            ...
                            <div className="space-y-2 md:space-y-3">
                              {review.title && <h4 className="font-black text-gray-900 text-[15px] md:text-base leading-snug">{review.title}</h4>}
                              <p className="text-[13px] md:text-sm text-gray-500 leading-relaxed font-medium">{review.content}</p>
                            </div>

                            {/* Images — Desktop: side panel style */}
                            <div className="hidden md:flex gap-2.5 mt-5 flex-wrap">
                              {reviewImages.slice(0, 4).map((img: string, idx: number) => (
                                <div key={idx} className="w-24 h-24 rounded-2xl overflow-hidden border-2 border-gray-100 shadow-sm hover:shadow-lg hover:-translate-y-1 hover:border-primary/30 transition-all duration-500 cursor-pointer group/img">
                                  <img src={img} alt="" className="w-full h-full object-cover group-hover/img:scale-110 transition-transform duration-500" />
                                </div>
                              ))}
                            </div>

                            {/* Images — Mobile: horizontal scroll strip */}
                            {reviewImages.length > 0 && (
                              <div className="flex md:hidden gap-2.5 mt-4 overflow-x-auto pb-1 scrollbar-hide -mx-1 px-1">
                                {reviewImages.map((img: string, idx: number) => (
                                  <div key={idx} className="w-24 h-24 rounded-2xl overflow-hidden border border-gray-100 flex-shrink-0 shadow-sm">
                                    <img src={img} alt="" className="w-full h-full object-cover" />
                                  </div>
                                ))}
                              </div>
                            ...
                            <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100/80">
                              <button 
                                ...
                                className="flex items-center gap-2 text-[10px] font-black text-gray-400 hover:text-primary active:scale-95 transition-all group/btn"
                              >
                                <ThumbsUp size={13} className="group-hover/btn:-translate-y-0.5 transition-transform" />
                                Helpful ({review.helpfulCount || 0})
                              </button>
                              <button className="text-[10px] font-bold text-gray-300 hover:text-red-400 uppercase tracking-wider transition-colors">Report</button>
                            </div>
                          </div>
                        );
                      })}
