  ...
  return (
    <div className="order-history">
      ...
      <div className="order-history-controls">
        ...
        <div className="order-history-filters custom-scrollbar">
          ...
          {STATUS_FILTERS.map((f) => {
            ...
            return (
              <button
                ...
                className={`order-filter-btn ${activeFilter === f ? 'order-filter-btn--active' : ''} status-filter--${f.toLowerCase()}`}
              >
                <span className="order-filter-label">{f}</span>
                {count > 0 && <span className="order-filter-badge">{count}</span>}
              </button>
            );
          })}
        </div>
