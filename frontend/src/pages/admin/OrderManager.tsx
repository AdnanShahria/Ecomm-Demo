    ...
    <div className="space-y-6">
      ...
      {selectedOrder && (
        <div className="fixed inset-0 z-40 bg-black/60 backdrop-blur-md flex items-end md:items-center justify-center p-0 md:p-4">
          <div className="bg-[var(--adm-card-bg)] rounded-[2rem] md:rounded-[2.5rem] w-full max-w-6xl h-[85vh] md:h-auto md:max-h-[95vh] flex flex-col shadow-2xl overflow-hidden animate-in slide-in-from-bottom md:zoom-in-95 duration-300 md:mb-0">
            {/* Header */}
            ...
            <div className="p-5 md:p-8 overflow-y-auto flex-grow bg-[var(--adm-card-bg)] custom-scrollbar pb-8 md:pb-8">
              {loadingDetails ? (
                ...
                <div className="lg:col-span-5 space-y-6">
                  ...
                  {/* Status Control */}
                  <div className="bg-primary text-white rounded-2xl md:rounded-[2rem] p-4 md:p-5 shadow-xl shadow-primary/20 relative overflow-hidden">
                    ...
                    <form onSubmit={handleStatusUpdate} className="space-y-3 relative z-10">
                        ...
                        <div className="sm:col-span-2">
                          ...
                          <select 
                            ...
                            onChange={(e) => setTrackingForm({...trackingForm, status: e.target.value})}
                             className="w-full px-4 py-2.5 rounded-xl bg-[var(--adm-card-bg)]/10 border border-white/20 text-white outline-none focus:bg-[var(--adm-card-bg)] focus:text-[var(--adm-text-primary)] transition-all text-xs md:text-sm font-bold appearance-none"
                        ...
                        <div className="sm:col-span-2">
                          ...
                          <input 
                            ...
                            placeholder="e.g. Dispatched from Warehouse"
                             className="w-full px-4 py-2.5 rounded-xl bg-[var(--adm-card-bg)]/10 border border-white/20 text-white placeholder:text-white/40 outline-none focus:bg-[var(--adm-card-bg)] focus:text-[var(--adm-text-primary)] transition-all text-xs md:text-sm font-medium"
                          />
                        </div>
                      </div>
                      <button 
                        type="submit"
                         className="w-full py-2.5 md:py-3 bg-accent text-white rounded-xl font-black uppercase tracking-widest hover:bg-[var(--adm-card-bg)] hover:text-accent transition-all shadow-lg text-[10px] md:text-xs"
                      >
                        Update & Notify
                      </button>
                    </form>
      ...
      {showInvoice && selectedOrder && (
        <Invoice 
          ...
          autoPrint={window.innerWidth < 768} 
        />
      )}
