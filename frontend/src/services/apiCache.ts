...
export function getResourceBase(url: string): string {
  ...
  // If last segment looks like an ID (is at position 4 or 5 and not a known resource name), pop it
  // Path examples:
  // /api/v1/products -> length 4, keep last
  // /api/v1/products/123 -> length 5, pop 123
  // /api/v1/users/abc/addresses -> length 6, pop addresses? No, we might want to pop the specific address ID if it were /addresses/id
  
  if (segments.length > 4) {
    const last = segments[segments.length - 1];
    // Popping if it's not a common resource name or if it looks like a hash/ID
    const commonResources = ['products', 'categories', 'orders', 'addresses', 'users', 'reviews', 'banners', 'settings', 'bulk', 'wallet', 'notifications'];
    if (!commonResources.includes(last.toLowerCase()) || last.includes('-') || last.includes('_') || /^[a-f0-9]{8,}$/i.test(last) || /^\d+$/.test(last)) {
      segments.pop();
    }
  }
