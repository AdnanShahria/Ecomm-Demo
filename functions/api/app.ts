// ═══════════════════════════════════════════════════════════════
// Aurelia — Consolidated API (Modular Version)
// ═══════════════════════════════════════════════════════════════
...
// DB Client Cache to avoid re-creation on every request
...
v1.use("*", async (c, next) => {
  ...
  // Demo Mode Protection: Block ALL modifying requests except auth
  const isDemoMode = String((c.env as any).DEMO_MODE) === "true";
  
  if (isDemoMode) {
    if (!path.startsWith("/api/v1/auth/")) {
      return c.json({
        error: "Forbidden",
        message: "Database modifications are disabled in Demo Mode."
      }, 403);
    }
  }
