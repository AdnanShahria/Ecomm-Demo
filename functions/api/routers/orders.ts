  ...
  try {
    ...
    if (!isAdmin && authHeader?.startsWith("Bearer ")) {
      try {
        ...
        loggedInUserId = payload.sub as string;
      } catch {}
    }

    if (!isAdmin && !loggedInUserId) {
      return c.json({ items: [], error: "Unauthorized access" }, 401);
    }
    
    const filterUserId = isAdmin ? userId : loggedInUserId;

    const rows = await db.query.orders.findMany({
      where: (o: any, { eq, and }: any) => {
        const filters = [];
        if (customerName && customerName.trim() !== "") filters.push(eq(o.customerName, customerName));
        if (filterUserId && filterUserId.trim() !== "") filters.push(eq(o.userId, filterUserId));
...
ordersRouter.get("/:id", async (c) => {
  ...
  // Protect registered user orders from guest tracking leaks
  if (!isAdmin && order.userId && order.userId !== loggedInUserId) {
    return c.json({ error: "Unauthorized", message: "You don't have permission to view this order." }, 401);
  }
